...
def create_buttons(text, text_font, text_color_normal, text_color_on_hover):
    # эта функция генерирует button objects using button text, button font, normal colour and color of the text когда
    # мышки находится на кнопке

    button_size = np.array([text_font[num].size(text[num]) for num in range(len(text))])
    # создание объектов кнопки

    buttons = [
        # текст, когда мышь находится за пределами диапазона кнопки
        [text_font[num].render(text[num], False, text_color_normal[num]),
         # текст, когда мышь находится внутри диапазона кнопки
...
def iterate_until_button_press(buttons, game_state, text_ending_place, text_starting_place):
    # пока кнопка не была нажата, этот метод проверяет, находится ли мышь в кнопке и если находится то она
    # меняет свой цвет
    button_clicked = 0
...
def draw_rects(button_size, buttons, game_state, text_starting_place, emit=list()):
    # рисуем прямоугольник вокруг текста кнопки

    for num in range(len(buttons)):
        game_state.canvas.surface.blit(
            buttons[num][0], text_starting_place[num])
        # emit содержит индексы кнопок, вокруг которых не нужен прямоугольник
